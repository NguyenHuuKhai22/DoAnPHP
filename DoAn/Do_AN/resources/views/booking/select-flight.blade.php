            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    @foreach($seatTypes as $key => $type)
                        <div class="border rounded-lg p-4 hover:shadow-md transition cursor-pointer seat-option {{ $key === 'pho_thong' ? 'border-teal-500' : '' }}" data-type="{{ $key }}">
                            <div class="font-semibold text-lg mb-2">{{ $type['name'] }}</div>
                            <div class="text-teal-700 font-bold text-xl mb-2">{{ number_format($flight->gia_ve_co_ban * $type['price_factor'], 0, ',', '.') }} VND</div>
                            <div class="text-sm text-gray-600">Giá cho 1 hành khách</div>
                            <div class="mt-4">
                                <input type="radio" name="seat_type" id="seat_{{ $key }}" value="{{ $key }}" {{ $key === 'pho_thong' ? 'checked' : '' }} class="hidden seat-radio">
                                <label for="seat_{{ $key }}" class="select-btn {{ $key === 'pho_thong' ? 'bg-teal-700 text-white' : 'bg-gray-200 text-gray-700' }} py-1 px-3 rounded text-center block w-full cursor-pointer transition">
                                    Chọn
                                </label>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const basePrice = {{ $flight->gia_ve_co_ban }};
        const numPassengers = {{ $soHanhKhach }};
        const priceFactors = {
            pho_thong: 1.0,
            pho_thong_dac_biet: 1.4,
            thuong_gia: 2.2
        };
        
        function updateTotalPrice(seatType) {
            const priceFactor = priceFactors[seatType];
        ...
        seatOptions.forEach(option => {
            option.addEventListener('click', function() {
                ...
                seatOptions.forEach(opt => {
                    ...
                    opt.querySelector('.seat-radio').checked = false;
                });
                ...
                this.querySelector('.seat-radio').checked = true;
                
                // Update total price
                updateTotalPrice(seatType);
            });
        });
    });
</script>
