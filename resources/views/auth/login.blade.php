    ...
    <div class="max-w-sm w-full bg-white rounded-lg shadow-2xl overflow-hidden transform transition-all duration-500 hover:scale-[1.02] animate-fadeIn">
        ...
        <div class="px-5 py-6">
            <form id="login-form" class="space-y-4">
                ...
                <div class="relative group">
                    <input type="password" name="password" id="mat_khau" required
                        class="block w-full pl-3 pr-3 py-2.5 border border-gray-300 rounded-md text-gray-900 text-sm placeholder-transparent focus:ring-2 focus:ring-[#0f766e] focus:border-transparent transition-all peer"
                        placeholder="{{ __('Mật khẩu') }}">
                    <label for="mat_khau" class="absolute left-3 -top-2.5 text-sm text-gray-600 transition-all bg-white px-1
                        peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-placeholder-shown:top-2.5
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    loginForm.addEventListener('submit', function(event) {
        ...
        .then(data => {
            if (data.success) {
                toastr.success("{{ __('Đăng nhập thành công!') }}", "{{ __('Thành công') }}");
                // Xóa lịch sử trình duyệt trước khi chuyển hướng
                window.history.replaceState({}, '', window.location.href);
                setTimeout(() => {
                    window.location.replace("{{ route('vietnam-airlines') }}");
                }, 1000);
            } else if (data.errors) {
                Object.values(data.errors).forEach(messages => {
                    messages.forEach(message => {
                        toastr.error(message, "{{ __('Lỗi') }}");
                    });
                });
                ...
                toastr.error(data.message, "{{ __('Lỗi') }}");
                // Remove loading state on error
                loginButton.classList.remove('loading');
                loginButton.disabled = false;
                // Re-enable form inputs
        ...
        .catch(error => {
            ...
            toastr.error("{{ __('Có lỗi xảy ra, vui lòng thử lại sau!') }}", "{{ __('Lỗi hệ thống') }}");
            // Remove loading state on error
            loginButton.classList.remove('loading');
            loginButton.disabled = false;
            // Re-enable form inputs
